    ...
    try {
        ...
        ffmpeg.on('log', ({ message }) => {
            ...
            if (m && window._duration) {
                const current = parseInt(m[1]) * 3600 + parseInt(m[2]) * 60 + parseFloat(m[3]);
                const pct = Math.min(99, Math.round((current / window._duration) * 100));
                setProgress(pct, `Procesando: ${m[0].replace('time=', '')}`);
            }
        });

        // Usar unpkg — sirve headers CORS correctos para Workers
...
function resetAll() {
    currentFile = null; outputBlob = null; window._duration = null;
